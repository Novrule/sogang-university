...
void list_destroy(struct list *list) {
  while (!list_empty(list)) {
    ...
    struct list_item *item = list_entry(elem, LIST_ITEM, elem);
    free(item);
  }
}
...
bool list_cmp(const struct list_elem *a, const struct list_elem *b, void *aux) {
...
void list_shuffle(struct list *list) {
  ...
  for (int i = 0; i < size - 1; i++) {
    struct list_elem *from = list_begin(list);
    struct list_elem *to = list_begin(list);
    int curIndex = i;
    int randIndex = rand() % (size - (i + 1)) + (i + 1);
