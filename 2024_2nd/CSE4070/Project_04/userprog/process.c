...
tid_t process_execute(const char *file_name) {
  ...
  while (e != list_end(&(cur_thread->child_list))) {
    ...
    if (child_thread->tid == tid) {
      sema_down(&child_thread->file_lock);

      for (int i = 2; i < 128; i++) {
	      if (cur_thread->fd[i] != NULL) {
		      child_thread->fd[i] = file_reopen(cur_thread->fd[i]);
		      if (cur_thread->fd[i]->deny_write) {
			      file_deny_write(child_thread->fd[i]);
		      }
	      }
      }
...
static void start_process(void *file_name_) {
  ...
  sema_up(&thread_current()->file_lock);
