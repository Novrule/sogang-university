...
void ofApp::draw() {
    ...
    if (draw_flag != 0) {
        ...
        ofSetColor(255, 0, 0);
        if (draw_flag == 2) {
            x = dot_arr[dot_index].x;
            y = dot_arr[dot_index].y;
            ofDrawCircle(x, y, 10);
        }
        else {
...
void ofApp::keyPressed(int key) {
    ...
    if (key == 's') {
        ...
        fall_flag = 1;
        water_arr = new water_dot[1000];
        int x_arr[100];
        float y_arr[100];
        int i, j, k, small_x, big_x, small_y, big_y, Big_x, Big_y;
        int flag = 0;
        int k_index = 9999;
        ...
        while (water_y != 728) {
            ...
            for (i = 0; i < line_num; i++) {
                small_x = line_arr[i].from_x;
                big_x = line_arr[i].from_x;
                big_y = line_arr[i].from_y;

                if (small_x > line_arr[i].to_x) {
                    small_x = line_arr[i].to_x;
                }
                else {
                    big_x = line_arr[i].to_x;
                }

                if (big_y < line_arr[i].to_y) {
                    big_y = line_arr[i].to_y;
                }
            ...
            if (j == 0) {
                ...
                small_y = 9999;
                for (k = 0; k < j; k++) {
                    y_arr[k] = line_arr[x_arr[k]].to_y - line_arr[x_arr[k]].from_y;
                    y_arr[k] = y_arr[k] / (line_arr[x_arr[k]].to_x - line_arr[x_arr[k]].from_x);
                    y_arr[k] = y_arr[k] * (water_x - line_arr[x_arr[k]].from_x) + line_arr[x_arr[k]].from_y;
                }

                for (k = 0; k < j; k++) {
                    if (y_arr[k] >= water_y && small_y > (int)y_arr[k]) {
                        small_y = (int)y_arr[k];
                        k_index = k;
                    }
                }
            ...
            if (water_y != 728) {
                ...
                Big_x = line_arr[x_arr[k_index]].from_x;
                Big_y = line_arr[x_arr[k_index]].from_y;

                if (Big_y < line_arr[x_arr[k_index]].to_y) {
                    Big_y = line_arr[x_arr[k_index]].to_y;
                    Big_x = line_arr[x_arr[k_index]].to_x;
                }
